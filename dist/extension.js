...
const _ = __webpack_require__(56);
class Utils {
    ...
    static processFileName(fileName) {
...
const file_system_manager_1 = __webpack_require__(5);
const base_model_1 = __webpack_require__(34);
const base_service_1 = __webpack_require__(36);
const base_view_model_1 = __webpack_require__(37);
const utils_1 = __webpack_require__(3);
const navigator_service_1 = __webpack_require__(38);
const locator_1 = __webpack_require__(39);
const logger_1 = __webpack_require__(40);
const providers_1 = __webpack_require__(41);
const main_1 = __webpack_require__(42);
const yaml_helper_1 = __webpack_require__(8);
class Architecture {
    ...
    init() {
        ...
        this.initViews();
        this.initWidgets();
        yaml_helper_1.YamlHelper.initializeWithDependencies();
        this.createExistingFile(this.rootPath, 'main.dart', new main_1.Main('main.dart').dartString);
    }
    initCore() {
        ...
        this.createFile(corePath, 'locator.dart', new locator_1.Locator('locator.dart').dartString);
        this.createFile(corePath, 'logger.dart', new logger_1.Logger('logger.dart').dartString);
        this.createFile(corePath, 'providers.dart', new providers_1.Providers('providers.dart').dartString);
    }
    initBase(corePath) {
        let basePath = path.join(corePath, 'base');
        let folderCreated = file_system_manager_1.FileSystemManager.createFolder(basePath);
        if (!folderCreated) {
            return;
        }
        this.createFile(basePath, 'base_model.dart', new base_model_1.BaseModel('base_model.dart').dartString);
        this.createFile(basePath, 'base_service.dart', new base_service_1.BaseService('base_service.dart').dartString);
        this.createFile(basePath, 'base_view_model.dart', new base_view_model_1.BaseViewModel('base_view_model.dart').dartString);
    }
    initServices(corePath) {
        let servicesPath = path.join(corePath, 'services');
        let folderCreated = file_system_manager_1.FileSystemManager.createFolder(servicesPath);
        if (!folderCreated) {
            return;
        }
        this.createFile(servicesPath, 'navigator_service.dart', new navigator_service_1.NavigatorService('navigator_service.dart').dartString);
    }
    initModels(corePath) {
        let modelsPath = path.join(corePath, 'models');
        let folderCreated = file_system_manager_1.FileSystemManager.createFolder(modelsPath);
        console.debug(`FolderCreated: ${folderCreated}`);
    }
    initTheme() {
        let themePath = path.join(this.rootPath, 'theme');
        let folderCreated = file_system_manager_1.FileSystemManager.createFolder(themePath);
        console.debug(`FolderCreated: ${folderCreated}`);
    }
    initViews() {
        let viewsPath = path.join(this.rootPath, 'views');
        let folderCreated = file_system_manager_1.FileSystemManager.createFolder(viewsPath);
        console.debug(`FolderCreated: ${folderCreated}`);
    }
    initWidgets() {
        let widgetsPath = path.join(this.rootPath, 'widgets');
        let folderCreated = file_system_manager_1.FileSystemManager.createFolder(widgetsPath);
        console.debug(`FolderCreated: ${folderCreated}`);
    }
    createFile(pathValue, fileName, data, options) {
        if (file_system_manager_1.FileSystemManager.doesFileExist(pathValue, fileName)) {
            console.error(`${fileName} already exists`);
            return;
        }
        file_system_manager_1.FileSystemManager.createFile(pathValue, fileName, data);
        utils_1.Utils.openFile(path.join(pathValue, fileName));
    }
    createExistingFile(pathValue, fileName, data, options) {
        file_system_manager_1.FileSystemManager.createFile(pathValue, fileName, data);
        utils_1.Utils.openFile(path.join(pathValue, fileName));
    }
}
...
const path = __webpack_require__(2);
const utils_1 = __webpack_require__(3);
const shell = __webpack_require__(Object(function webpackMissingModule() { var e = new Error("Cannot find module 'shelljs'"); e.code = 'MODULE_NOT_FOUND'; throw e; }()));
const vs_code_actions_1 = __webpack_require__(7);
const yaml_helper_1 = __webpack_require__(8);
class FileSystemManager {
    static createFile(pathValue, fileName, data) {
        let filePath = path.join(pathValue, fileName);
        (0, fs_1.writeFileSync)(filePath, data);
        utils_1.Utils.openFile(filePath);
    }
    static createFolder(pathValue) {
    ...
    static doesFileExist(filePath, fileName) {
        return (0, fs_1.existsSync)(path.join(filePath, fileName));
    }
    static readFileAsString(filePath, fileName) {
        if (!this.doesFileExist(filePath, fileName)) {
            return undefined;
        }
        let fileBuffer = (0, fs_1.readFileSync)(path.join(filePath, fileName));
        let fileData = fileBuffer.toString();
        return fileData;
    }
    static isFlutterProject() {
        let rootPath = vs_code_actions_1.VsCodeActions.rootPath;
        if (!(0, fs_1.existsSync)(path.join(rootPath, 'pubspec.yaml'))) {
            vs_code_actions_1.VsCodeActions.showErrorMessage('Pubspec.yaml not found');
            return false;
        }
        let errorMessage = yaml_helper_1.YamlHelper.isValidFlutterPubspec();
        console.error(errorMessage);
        if (errorMessage !== undefined) {
            vs_code_actions_1.VsCodeActions.showErrorMessage(errorMessage);
            return false;
        }
        return true;
    }
...
class VsCodeActions {
     ...
     */
    static async getInputString(placeHolder, validateInput) {
        let input = await vscode_1.window.showInputBox({
...
const yaml = __webpack_require__(9);
const vs_code_actions_1 = __webpack_require__(7);
const file_system_manager_1 = __webpack_require__(5);
class YamlHelper {
    static initializeWithDependencies() {
        this.upgradeDartVersion();
        this.addDependencyToPubspec("stacked", "3.0.0");
        this.addDependencyToPubspec("responsive_builder", "0.1.4");
        this.addDependencyToPubspec("provider", "6.0.0");
        this.addDependencyToPubspec("logger", "1.1.0");
        this.addDependencyToPubspec("get_it", "7.2.0");
        this.addDependencyToPubspec("equatable", "2.0.0");
    ...
    static upgradeDartVersion() {
        ...
        let object = JSON.parse(json);
        object["environment"]["sdk"] = ">=2.17.0 <3.0.0";
        let modifiedString = JSON.stringify(object);
  ...
  # see https://flutter.dev/custom-fonts/#from-packages`;
    ...
    static getPubspecJsonFile() {
        let rootPath = vs_code_actions_1.VsCodeActions.rootPath;
        let fileData = file_system_manager_1.FileSystemManager.readFileAsString(rootPath, "pubspec.yaml");
    ...
    static overwritePubspecFile(data) {
        file_system_manager_1.FileSystemManager.createFile(vs_code_actions_1.VsCodeActions.rootPath, "pubspec.yaml", data);
    }
    static toYAML(text) {
        let json;
        try {
            ...
            vs_code_actions_1.VsCodeActions.showErrorMessage("Could not parse the selection as JSON.");
            console.error(e);
            return undefined;
        }
    ...
    static toJSON(text) {
        ...
        try {
            ...
            vs_code_actions_1.VsCodeActions.showErrorMessage("Could not parse the selection as YAML.");
            console.error(e);
            return;
        }
    ...
    static getIndent() {
        const editorCfg = vs_code_actions_1.VsCodeActions.getEditorConfiguration();
...
module.exports.DEFAULT_SCHEMA = __webpack_require__(14);
module.exports.load                = loader.load;
module.exports.loadAll             = loader.loadAll;
module.exports.dump                = dumper.dump;
module.exports.YAMLException = __webpack_require__(12);

// Re-export all types in case user wants to create custom schema
module.exports.types = {
  binary:    __webpack_require__(29),
  float:     __webpack_require__(26),
  map:       __webpack_require__(22),
  null:      __webpack_require__(23),
  pairs:     __webpack_require__(31),
  set:       __webpack_require__(32),
  timestamp: __webpack_require__(27),
  bool:      __webpack_require__(24),
  int:       __webpack_require__(25),
  merge:     __webpack_require__(28),
  omap:      __webpack_require__(30),
  seq:       __webpack_require__(21),
  str:       __webpack_require__(20)
};
...
var YAMLException       = __webpack_require__(12);
var makeSnippet         = __webpack_require__(13);
var DEFAULT_SCHEMA      = __webpack_require__(14);
...
const base_1 = __webpack_require__(35);
class BaseModel extends base_1.Base {
    constructor(fileName, suffix) {
...
const _ = __webpack_require__(56);
class Base {
    constructor(fileName, classSuffix) {
...
const base_1 = __webpack_require__(35);
class BaseService extends base_1.Base {
    constructor(fileName, suffix) {
        super(fileName, suffix);
...
const base_1 = __webpack_require__(35);
class BaseViewModel extends base_1.Base {
    constructor(fileName, suffix) {
        super(fileName, suffix);
...
class BaseViewModel extends ChangeNotifier {
  ...
  BaseViewModel({
    ...
    String? title,
      ...
      _title = title {
    log = getLogger(title ?? title.runtimeType.toString());
  }

  bool get busy => _busy;
  bool get isDisposed => this._isDisposed;
  String? get title => _title ?? _title.runtimeType.toString();
  ...
  void notifyListeners() {
    if (!isDisposed) {
        ...
        log?.w('notifyListeners: Notify listeners called after '
            '\${title ?? title.runtimeType.toString()} has been disposed');
    }
  }
...
const base_1 = __webpack_require__(35);
class NavigatorService extends base_1.Base {
    constructor(fileName, suffix) {
        super(fileName, suffix);
...
const base_1 = __webpack_require__(35);
class Locator extends base_1.Base {
    constructor(fileName, suffix) {
        super(fileName, suffix);
...
const base_1 = __webpack_require__(35);
class Providers extends base_1.Base {
    constructor(fileName, suffix) {
        super(fileName, suffix);
...
const view_model_1 = __webpack_require__(45);
const mobile_1 = __webpack_require__(46);
const desktop_1 = __webpack_require__(47);
const tablet_1 = __webpack_require__(48);
const yaml_helper_1 = __webpack_require__(8);
class ViewFile {
    constructor(rootPath, fileName, folders) {
        this.rootPath = rootPath;
        this.fileName = fileName;
        this.folders = folders;
        console.debug(`ViewFile(rootPath: ${rootPath}, fileName: ${fileName})`);
        let folderCreated = file_system_manager_1.FileSystemManager.createFolder(this.pathValue);
        if (!folderCreated) {
            return;
        }
    }
    createResponsiveViews(options) {
        this.createFiles(this.snakeCasedFileName + '_view.dart', new view_1.View(this.snakeCasedFileName, 'View').dartString);
    ...
    createDemoViews() {
        this.createFiles(this.snakeCasedFileName + '_view.dart', new view_1.View(this.snakeCasedFileName, 'View').dartString);
        this.createFiles(this.snakeCasedFileName + '_mobile.dart', new mobile_1.Mobile(this.snakeCasedFileName, 'Mobile').demoString);
        this.createFiles(this.snakeCasedFileName + '_desktop.dart', new desktop_1.Desktop(this.snakeCasedFileName, 'Desktop').demoString);
        this.createFiles(this.snakeCasedFileName + '_tablet.dart', new tablet_1.Tablet(this.snakeCasedFileName, 'Tablet').demoString);
        this.createFiles(this.snakeCasedFileName + '_view_model.dart', new view_model_1.ViewModel(this.snakeCasedFileName, 'ViewModel', yaml_helper_1.YamlHelper.getProjectName()).demoString);
    }
    createView() {
    }
    createMobile() {
        this.createFiles(this.snakeCasedFileName + '_mobile.dart', new mobile_1.Mobile(this.snakeCasedFileName, 'Mobile').dartString);
    }
    createDesktop() {
        this.createFiles(this.snakeCasedFileName + '_desktop.dart', new desktop_1.Desktop(this.snakeCasedFileName, 'Desktop').dartString);
    }
    createTablet() {
        this.createFiles(this.snakeCasedFileName + '_tablet.dart', new tablet_1.Tablet(this.snakeCasedFileName, 'Tablet').dartString);
    }
    createWithViewModel() {
        this.createFiles(this.snakeCasedFileName + '_view_model.dart', new view_model_1.ViewModel(this.snakeCasedFileName, 'ViewModel', yaml_helper_1.YamlHelper.getProjectName()).dartString);
    }
    get snakeCasedFileName() {
    ...
    createFiles(fileName, data) {
        if ((0, fs_1.existsSync)(path.join(this.pathValue, this.snakeCasedFileName))) {
            console.warn(`${fileName} already exists`);
            return;
        }
        file_system_manager_1.FileSystemManager.createFile(this.pathValue, fileName, data);
    }
}
...
const _ = __webpack_require__(56);
const base_1 = __webpack_require__(35);
class View extends base_1.Base {
    constructor(fileName, suffix) {
        super(fileName, suffix);
        let classPrefixList = this.className.split('View');
        let classPrefix;
...
const base_1 = __webpack_require__(35);
class ViewModel extends base_1.Base {
    constructor(fileName, suffix, projectName) {
        super(fileName, suffix);
...
const _ = __webpack_require__(56);
const base_1 = __webpack_require__(35);
class Mobile extends base_1.Base {
    constructor(fileName, suffix) {
        super(fileName, suffix);
        let classPrefixList = this.className.split('Mobile');
        let classPrefix;
        if (!_.isEmpty(classPrefixList)) {
            classPrefix = _.first(classPrefixList);
        }
...
const _ = __webpack_require__(56);
const base_1 = __webpack_require__(35);
class Desktop extends base_1.Base {
    constructor(fileName, suffix) {
        super(fileName, suffix);
        let classPrefixList = this.className.split('Desktop');
        let classPrefix;
        if (!_.isEmpty(classPrefixList)) {
            classPrefix = _.first(classPrefixList);
        }
...
const _ = __webpack_require__(56);
const base_1 = __webpack_require__(35);
class Tablet extends base_1.Base {
    constructor(fileName, suffix) {
        super(fileName, suffix);
        let classPrefixList = this.className.split('Tablet');
        let classPrefix;
        if (!_.isEmpty(classPrefixList)) {
            classPrefix = _.first(classPrefixList);
        }
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.WidgetFile = void 0;
const path = __webpack_require__(2);
const _ = __webpack_require__(56);
const fs_1 = __webpack_require__(6);
const file_system_manager_1 = __webpack_require__(5);
const view_model_1 = __webpack_require__(50);
const mobile_1 = __webpack_require__(51);
const desktop_1 = __webpack_require__(52);
const tablet_1 = __webpack_require__(53);
const widget_1 = __webpack_require__(54);
class WidgetFile {
    ...
    createResponsiveWidgets() {
        this.createFiles(this.snakeCasedFileName + '_widget.dart', new widget_1.Widget(this.snakeCasedFileName, 'Widget').dartString);
        this.createMobile();
        this.createTablet();
        this.createDesktop();
    }
    ...
    createWithViewModel() {
        this.createFiles(this.snakeCasedFileName + '_view_model.dart', new view_model_1.ViewModel(this.snakeCasedFileName, 'ViewModel').dartString);
    }
    get snakeCasedFileName() {
        let snakeCasedFileName = _.snakeCase(this.fileName);
        console.debug(`get snakeCasedFileName: ${snakeCasedFileName}`);
    ...
    get pathValue() {
        return path.join(this.rootPath, 'lib', 'widgets', this.snakeCasedFileName);
    }
    createFiles(fileName, data) {
        if ((0, fs_1.existsSync)(path.join(this.pathValue, this.snakeCasedFileName))) {
            console.warn(`${fileName} already exists`);
...
const _ = __webpack_require__(56);
const base_1 = __webpack_require__(35);
class Widget extends base_1.Base {
    constructor(fileName, suffix) {
        super(fileName, suffix);
        let classPrefixList = this.className.split('Widget');
        let classPrefix;
        if (!_.isEmpty(classPrefixList)) {
            classPrefix = _.first(classPrefixList);
        }
...
const _ = __webpack_require__(56);
const utils_1 = __webpack_require__(3);
const architecture_1 = __webpack_require__(4);
const view_file_1 = __webpack_require__(43);
const vs_code_actions_1 = __webpack_require__(7);
const widget_file_1 = __webpack_require__(49);
const file_system_manager_1 = __webpack_require__(5);
function activate(context) {
    ...
    let initializeDisposable = vscode.commands.registerCommand("extension.initializeArchitecture", async () => {
        if (!file_system_manager_1.FileSystemManager.isFlutterProject()) {
            return;
        }
        let rootPath = vs_code_actions_1.VsCodeActions.rootPath;
        if (_.isUndefined(rootPath)) {
        ...
        new architecture_1.Architecture(path.join(rootPath, "lib")).init();
        new view_file_1.ViewFile(rootPath, "home").createDemoViews();
    });
    let viewDisposable = vscode.commands.registerCommand("extension.createViews", async () => {
        if (!file_system_manager_1.FileSystemManager.isFlutterProject()) {
            return;
        }
        let inputString = await vs_code_actions_1.VsCodeActions.getInputString("Enter class name", async (value) => {
            if (value.length === 0) {
        ...
        if (inputString.length === 0 || inputString.toLowerCase() === "view") {
            ...
            vs_code_actions_1.VsCodeActions.showErrorMessage("Invalid name for file");
            return;
        }
        ...
        let opts = await vs_code_actions_1.VsCodeActions.showQuickPick();
        let options = await vs_code_actions_1.VsCodeActions.getInputString("Enter options => 0 for all, 1 for mobile, 2 for web and 3 for tablet. Separate with commas.", async (value) => {
        ...
        let fileName = utils_1.Utils.processFileName(formattedInputString);
        console.debug(`activate: fileName: ${fileName}`);
        let rootPath = vs_code_actions_1.VsCodeActions.rootPath;
        if (rootPath === undefined) {
            return;
        }
        new view_file_1.ViewFile(rootPath, fileName, folders).createResponsiveViews(options);
    });
    let widgetDisposable = vscode.commands.registerCommand("extension.createWidget", async () => {
        ...
        let fileName = utils_1.Utils.processFileName(inputString.trim());
        console.debug(`activate: fileName: ${fileName}`);
        let rootPath = vs_code_actions_1.VsCodeActions.rootPath;
        if (rootPath === undefined) {
            return;
        }
        new widget_file_1.WidgetFile(rootPath, fileName).createResponsiveWidgets();
    });
